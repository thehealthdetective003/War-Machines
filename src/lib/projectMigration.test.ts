...
const initial = { projectSchemaVersion: 12, projectName: 'Untitled', projectFormat: 'standard-lifecycle', phase: 1, topic: null, plannedScenes:[], sceneDirections: [], masterVoiceoverScript: '', voiceoverTranscription: null, t2vPromptProfile:'omni-flash', visualPrompts: [], demoState: 'idle', demoScenes: [], demoSceneNumbers: [], generationSession:idleGenerationSession() } as AppState;
...
test('round-trips a complete 8-second T2V project without changing its timeline', () => {
  ...
  assert.equal(result.state?.projectSchemaVersion, 12);
...
test('schema 11 projects preserve inputs but reset downstream output for automatic realignment',()=>{
  ...
  const result=migrateProject(raw,initial,8);assert.equal(result.state?.projectSchemaVersion,12);assert.equal(result.state?.phase,2);assert.deepEqual(result.state?.plannedScenes,[]);assert.deepEqual(result.state?.sceneDirections,[]);assert.deepEqual(result.state?.visualPrompts,[]);assert.match(result.message||'',/realignment/i);
});
