...
const project=(id:string,count:number):AppState=>({id,projectSchemaVersion:10,projectName:id,projectFormat:'standard-lifecycle',phase:3,topic:null,plannedScenes:[],sceneDirections:[],masterVoiceoverScript:'',voiceoverTranscription:null,t2vPromptProfile:'omni-flash',visualPrompts:Array.from({length:count},(_,index)=>prompt(index+1)),demoState:'idle',demoScenes:[],demoSceneNumbers:[],generationSession:{...idleGenerationSession(),completedScenes:count}});
