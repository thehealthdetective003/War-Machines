...
export interface ApiUsageDiagnostics {
  ...
  locallyCompiledGraphicPrompts:number;
  unusedGeneratedFieldsRemoved:number;
  estimatedInputCharactersAvoided:number;
}

const empty=():ApiUsageDiagnostics=>({totalApiCalls:0,alignmentApiCalls:0,directionApiCalls:0,promptApiCalls:0,correctionApiCalls:0,totalRequestedItems:0,firstPassAcceptedItems:0,selectivelyRetriedItems:0,fullBatchRetriesAvoided:0,duplicateCallsAvoided:0,persistedResultsReused:0,locallyCompiledGraphicPrompts:0,unusedGeneratedFieldsRemoved:0,estimatedInputCharactersAvoided:0});
let metrics=empty();
...
export function recordLocalGraphicCompilation(count=1){metrics.locallyCompiledGraphicPrompts+=count;debug();}
