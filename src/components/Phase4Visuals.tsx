...
export function Phase4Visuals({ state, setState, commitProjectState, resumeAfterRecovery, onResumeAfterRecoveryConsumed }: Props) {
  ...
  const requestPrompts = async (ai: GoogleGenAI, selected: typeof directions, qualityRetry?:string): Promise<T2VPrompt[]> => {
    ...
    const prompts=selected.map(direction => { const item=byNumber.get(direction.number);
    ...
    const audited=prompts.map((prompt,index)=>({...prompt,quality_flags:promptQualityIssues(prompt,selected[index])}));
    const failures=audited.filter(prompt=>prompt.quality_flags.length);if(failures.length){const summary=`scene${failures.length>1?'s':''} ${failures.map(prompt=>prompt.number).join(', ')}: ${failures.flatMap(prompt=>prompt.quality_flags).filter((value,index,list)=>list.indexOf(value)===index).join(', ')}`;if(!qualityRetry){const failedNumbers=new Set(failures.map(prompt=>prompt.number)),retryDirections=selected.filter(direction=>failedNumbers.has(direction.number)),corrected=await requestPrompts(ai,retryDirections,summary),correctedByNumber=new Map(corrected.map(prompt=>[prompt.number,prompt]));return audited.map(prompt=>correctedByNumber.get(prompt.number)||prompt);}const blocking=failures.filter(prompt=>prompt.quality_flags.some(issue=>!isAdvisoryPromptQualityIssue(issue)));if(!blocking.length){console.warn('Advisory prompt-quality diagnostics remained after one automatic correction; continuing with structurally valid prompts.',summary);return audited;}throw new Error(`Automatic prompt quality gate rejected ${summary} after one corrective retry.`);}
    return audited;
  };
